...
public class StartExperience : MonoBehaviour
{
    ...
    {
        if(other.tag == "Player1") player1Trigger = true;
        if(other.tag == "Player2") player2Trigger = true;
        Debug.Log(other.tag);
    }

        private void OnTriggerExit(Collider other)
    {
        if(other.tag == "Player1") player1Trigger = false;
        if(other.tag == "Player2") player2Trigger = false;
    }
